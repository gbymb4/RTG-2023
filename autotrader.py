...
class AutoTrader(BaseAutoTrader):
    ...
    def __init__(self, loop: asyncio.AbstractEventLoop, team_name: str, secret: str):
        ...
        self.last_hedge_side = None
        self.last_etf_side = None
    ...
    def on_order_book_update_message(self, instrument: int, sequence_number: int, ask_prices: List[int],
        ...
        if instrument == Instrument.ETF:
            ...
            instrument_name = 'hedge'
            
        self.history[f'{instrument_name}_bids'].append(bid_prices)
        self.history[f'{instrument_name}_asks'].append(ask_prices)
            
        self.history[f'{instrument_name}_bids_volumes'].append(bid_volumes)
        self.history[f'{instrument_name}_asks_volumes'].append(ask_volumes)
            
        self.history[f'{instrument_name}_mid_prices'].append(mid_price)
    ...
    def on_order_filled_message(self, client_order_id: int, price: int, volume: int) -> None:
        ...
        if client_order_id in self.bids:
            ...
            self.ask_id = next(self.order_ids)
            self.send_hedge_order(self.ask_id, Side.ASK, MIN_BID_NEAREST_TICK, volume)
            self.hedge_asks.add(self.ask_id)
        elif client_order_id in self.asks:
            self.position -= volume
            self.bid_id = next(self.order_ids)
            self.send_hedge_order(self.bid_id, Side.BID, MAX_ASK_NEAREST_TICK, volume)
            self.hedge_bids.add(self.bid_id)
    ...
    def send_hedge_order(self, client_order_id: int, side: Side, price: int, volume: int) -> None:
        ...
        self.last_hedge_side = side
        super().send_hedge_order(client_order_id, side, price, volume)
